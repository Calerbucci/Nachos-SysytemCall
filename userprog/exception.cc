               ...
               case SC_Open:
                ...
                case SC_Write:
                        val = kernel->machine->ReadRegister(4);
                        {
                                ...
                                int value = kernel->fileSystem->WriteFile(buffer, kernel->machine->ReadRegister(5));
                ...
                case SC_Read:
                        ...
                        {
                                char *buffer = &(kernel->machine->mainMemory[val]);
                                int value = kernel->fileSystem->ReadFile(buffer, kernel->machine->ReadRegister(5));
                                kernel->machine->WriteRegister(2, (int)value);
                        }
                        return;
                        ASSERTNOTREACHED();
                ...
                case SC_Close:
                        val = kernel->machine->ReadRegister(4);
                        {
                                ...
                                int status = kernel->fileSystem->CloseFile();
                                kernel->machine->WriteRegister(2, (int)status);
                        }
                        return;
                        ASSERTNOTREACHED();
